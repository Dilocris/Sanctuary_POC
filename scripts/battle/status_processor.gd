...
func process_end_of_turn_effects(actor: Character) -> void:
	...
	for status in actor.status_effects:
		...
		if tags.has("DOT"):
			...
			_battle_manager._add_limit_on_damage_taken(actor, value, _battle_manager.LIMIT_GAIN_DOT_DIV)
			_battle_manager.add_message(actor.display_name + " takes " + str(value) + " damage.")
			_battle_manager.emit_signal("status_tick", actor.id, value, "DOT")
		if tags.has("HOT"):
			actor.heal(value)
